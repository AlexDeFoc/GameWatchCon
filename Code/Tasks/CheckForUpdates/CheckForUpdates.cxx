...
auto gw::tasks::CheckForUpdates(Console& console) noexcept -> TaskType {
    ...
    console.WriteLineToCache(Console::MsgType::Info, std::format("Current version: {}.{}.{}", current_version.Year, current_version.Month, current_version.Day));

    if (!CanUserConnectToTheInternet()) {
        console.WriteLineToCache(Console::MsgType::Error, "It looks like you're disconnected from the internet");
    ...
    if (online_version_as_text == std::nullopt) {
        console.WriteLineToCache(Console::MsgType::Error, "Something went wrong while connecting to the repository from GitHub");
        return TaskType::MainMenu;
    }
    ...
    if (online_version == std::nullopt) {
        console.WriteLineToCache(Console::MsgType::Error, "Version file on the GitHub repo isn't formatted correctly");
        return TaskType::MainMenu;
    }

    console.WriteLineToCache(Console::MsgType::Info, std::format("Online version: {}.{}.{}", online_version->Year, online_version->Month, online_version->Day));

    if (IsThereAnUpdateAvailable(current_version, *online_version))
        console.WriteLineToCache(Console::MsgType::Tip, "There exist available updates");
    else
        console.WriteLineToCache(Console::MsgType::Tip, "There are no available updates");
...
auto ExtractVersionFromText(const std::string_view ver_text) noexcept -> std::optional<Version> {
    ...
    auto [ptr1, ec1] = std::from_chars(start, end, v.Year);
    if (ec1 != std::errc{} || ptr1 == end || *ptr1 != '.') {
        return std::nullopt;
    }

    auto [ptr2, ec2] = std::from_chars(ptr1 + 1, end, v.Month);
    if (ec2 != std::errc{} || ptr2 == end || *ptr2 != '.') {
        return std::nullopt;
    }

    auto [ptr3, ec3] = std::from_chars(ptr2 + 1, end, v.Day);
    if (ec3 != std::errc{}) {
        return std::nullopt;
    }

    // Trailing junk after the day
    if (ptr3 != end)
        return std::nullopt;
