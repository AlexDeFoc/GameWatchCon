...
ChangeGameTitle::ChangeGameTitle(const std::shared_ptr<Context>& ctx) noexcept : Task{ctx}, game_library_{Task::ctx->game_library}, console_{Task::ctx->console} {}
...
auto ChangeGameTitle::Run() noexcept -> std::unique_ptr<Task> {
    ...
    switch (console_.GetInputRequestStatus()) {
        ...
        case ConsoleComponents::InputRequestStatus::Invalid:
            console_.WriteLineToCache(ConsoleComponents::MsgType::Info, "An issue has occurred with changing game title");
            return std::make_unique<GetEditGamesMenuOptionChoice>(ctx);

        default:
            assert(false && "Unhandled ConsoleComponents::InputRequestStatus in Task_ChangeGameTitle::Run after requesting game id");
            std::terminate();
    }
    ...
    switch (console_.GetInputRequestStatus()) {
        case ConsoleComponents::InputRequestStatus::Success:
            ...
            console_.WriteLineToCache(ConsoleComponents::MsgType::Info, "Game title changed");
            return std::make_unique<GetEditGamesMenuOptionChoice>(ctx);
