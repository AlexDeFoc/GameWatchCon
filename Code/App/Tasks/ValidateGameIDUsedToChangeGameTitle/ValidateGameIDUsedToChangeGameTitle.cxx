...
module Task_ValidateGameIDUsedToChangeGameTitle;
...
ValidateGameIDUsedToChangeGameTitle::ValidateGameIDUsedToChangeGameTitle(const std::shared_ptr<Context>& ctx) noexcept : Task{ctx}, console_{Task::ctx->console} {}

auto ValidateGameIDUsedToChangeGameTitle::Run() noexcept -> std::unique_ptr<Task> {
    switch (console_.GetInputRequestStatus()) {
        case ConsoleComponents::InputRequestStatus::Success:
            return std::make_unique<GetGameTitleUsedToChangeGameTitle>(ctx);
