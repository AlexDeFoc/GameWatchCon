...
public:
    ...
    /*!
     * @brief Requests from the user a new auto save interval amount in seconds
     ...
     * @return Interval in seconds & Input Status pair
     */
    [[nodiscard]] auto RequestNewAutoSaveInterval(std::string, bool = true) -> std::pair<std::chrono::seconds, InputStatus>;
     ...
     */
    auto ThrowOnUnhandledCase(std::string_view = {}, std::source_location = std::source_location::current()) const -> void;
     ...
     */
    auto ThrowOnUnhandledCase(std::source_location, std::string_view = {}, std::source_location = std::source_location::current()) const -> void;
    ...
    [[nodiscard]] auto RequestUserGameIDChoice(ListingFunc, std::size_t, bool = true, std::size_t = 0, bool = true) -> std::pair<std::size_t, InputStatus>;
...
template <typename ListingFunc>
auto gw::Console::RequestUserGameIDChoice(ListingFunc list_games_func, std::size_t games_count, bool is_active_game_id_valid_choice, std::size_t active_game_id, bool is_request_cancellable) -> std::pair<std::size_t, InputStatus> {
    while (true) {
        ClearCout();
        list_games_func();
        WriteCachedMsgs();
        ...
        if (std::string input{}; std::getline(std::cin, input)) {
            try {
                std::size_t selected_game_id = std::stoull(input);
