...
// Public
gw::DiskManager::DiskManager(Console& console) : console_{console}, storage_{InitStorage()}, storage_path_{GetStoragePath()}, storage_backup_path_{GetStorageBackupPath()} {
    ...
    try {
            ...
            switch (result) {
                ...
                case sync_schema_result::new_columns_added_and_old_columns_removed:
                    if (!UserIsSureToDropColumns()) {
                        ...
                        console.WriteLine(Console::Tag::FatalError, "Attempted to load '{}' but app was about to remove columns, thus it has crashed. Reload the program to confirm this action, else fix the database.", storage_filename_);
                        console.WriteLine(Console::Tag::Tip, "Press any key to proceed");
                        console.WriteLine(Console::Tag::Info, "The app will now exit");
                ...
                default:
                    console.ThrowOnUnhandledCase("Unhandled sqlite-orm case");
            }
        }
        ...
        if (err.code().value() == SQLITE_BUSY || err.code().value() == SQLITE_LOCKED) {
            DeleteTempBackupStorageFile();
            console.WriteLine(Console::Tag::FatalError, "Attempted to load '{}' but found the database is in use! Open the app when the database is not in use by another process!", storage_filename_);
            console.WriteLine(Console::Tag::Tip, "Press any key to proceed");
            console.WriteLine(Console::Tag::Info, "The app will now exit");
            console.RequestKeyPress();
            throw;
            ...
            DeleteTempBackupStorageFile();
            console.WriteLine(Console::Tag::FatalError, "Attempted to load '{}' but found either not a valid sqlite3 database OR the database is corrupt! Open the app when you have removed the invalid database or recovered it!", storage_filename_);
            console.WriteLine(Console::Tag::Tip, "Press any key to proceed");
            console.WriteLine(Console::Tag::Info, "The app will now exit");
            console.RequestKeyPress();
            throw;
            ...
            DeleteTempBackupStorageFile();
            console.ThrowOnUnhandledCase(std::source_location::current(), "Sqlite-orm error occured: '{}'", err.what());
        }
    } catch (const std::exception& err) {
        DeleteTempBackupStorageFile();
        console.ThrowOnUnhandledCase(std::source_location::current(), "Unknown error occured: '{}'", err.what());
    }
}

auto gw::DiskManager::GetAutoSaveEnabledStatusValue() noexcept -> std::size_t {
    try {
...
auto gw::DiskManager::GetAutoSaveIntervalValue() const noexcept -> std::size_t {
    try {
        auto result = storage_->select(&SettingsInStorage::auto_save_interval_in_sec, sqlite_orm::limit(1));
...
auto gw::DiskManager::GetGameTitle(const std::size_t game_id) const noexcept -> std::string {
    try {
...
auto gw::DiskManager::GameCount() const noexcept -> std::size_t {
    try {
...
auto gw::DiskManager::SetAutoSaveIntervalValue(std::size_t new_interval) const noexcept -> void {
    if (auto settings_ptr = storage_->get_no_throw<SettingsInStorage>(1)) {
        settings_ptr->auto_save_interval_in_sec = new_interval;
        storage_->update(*settings_ptr);
    }
}

// TODO: Optimize process
auto gw::DiskManager::SetGameTitle(const std::size_t game_id, std::string&& game_title) const noexcept -> void {
    try {
        int offset_val = static_cast<int>(game_id - 1);
...
// TODO: Optimize process
auto gw::DiskManager::ResetGamePlaytime(const std::size_t game_id) const noexcept -> void {
    try {
        int offset_val = static_cast<int>(game_id - 1);
...
auto gw::DiskManager::DeleteGame(const std::size_t game_id) const noexcept -> void {
    try {
        int offset_val = static_cast<int>(game_id - 1);
...
auto gw::DiskManager::GetExeDirPath() const noexcept -> std::string {
    ...
    do {
        ...
        auto actual_path_length = ::GetModuleFileNameW(NULL, wide_path.data(), path_length_capacity);
    ...
    std::string utf8_path(utf8_bytes_needed - 1, '\0'); // -1 to exclude null terminator
