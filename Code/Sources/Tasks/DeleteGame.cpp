...
// TODO: Remove these into a Utils.h, and all copies & similar ones
namespace {
[[nodiscard]] auto GetFormattedPlaytime(std::chrono::milliseconds) noexcept -> std::string;
} // namespace

auto gw::tasks::DeleteGame(gw::Console& console, gw::AppState&, gw::AppSettings&, gw::GameLibrary& game_library) -> void* {
    ...
    const auto [selected_game_id, input_status_for_game_id] = console.RequestUserGameIDChoice(list_games_func, game_library.GetGameCount(), false, game_library.GetActiveGameId());
    ...
    if (user_is_sure) {
        game_library.DeleteGame(selected_game_id);
        console.WriteLineToCache(Console::Tag::Success, "Deleted game"); // TODO: Add more context
    } else
        console.WriteLineToCache(Console::Tag::Info, "Action cancelled");
