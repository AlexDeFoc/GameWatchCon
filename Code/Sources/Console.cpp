...
auto gw::Console::RequestNewAutoSaveInterval(std::string formatted_current_interval, const bool is_request_cancellable) -> std::pair<std::chrono::seconds, InputStatus> {
    while (true) {
        ClearCout();
        WriteCachedMsgs();
        ...
        Write(Tag::Request, "Enter new interval (in seconds): ");
        ...
        if (std::string input{}; std::getline(std::cin, input)) {
            try {
                ...
                if (new_interval == 0) {
                    WriteLineToCache(Tag::Error, "Interval cannot be 0!");
                    continue;
                } else if (new_interval < 0) {
                    WriteLineToCache(Tag::Error, "Cannot have a negative interval!");
                    continue;
                } else
                    return {std::chrono::seconds(new_interval), InputStatus::Success};
            } catch (const std::invalid_argument&) {
                WriteLineToCache(Tag::Error, "Invalid input!");
                continue;
            } catch (const std::out_of_range&) {
            ...
            std::cin.clear();
            if (is_request_cancellable) {
                return {std::chrono::seconds::zero(), InputStatus::Cancelled};
            } else {
                WriteLineToCache(Tag::Error, "Invalid input!");
                continue;
            }
...
auto gw::Console::RequestKeyPress() noexcept -> void {
    ...
    [&] {
        ...
        if (::SetConsoleMode(std_in_handle, cin_mode_original & (~(ENABLE_LINE_INPUT | ENABLE_ECHO_INPUT))) == 0) {
            failed = true;
            return;
        }
...
auto gw::Console::WriteCachedMsgs() noexcept -> void {
    std::fputs(cached_msgs_.c_str(), stdout);
    cached_msgs_.clear();
}
